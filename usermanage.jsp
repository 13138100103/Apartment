 ...
 <!--data-toggle collapse 折叠  ul class =collapse 初始折叠 -->
    ...
    <i class="glyphicon glyphicon-cog"></i>后台管理 <span class="pull-right glyphicon glyphicon-chevron-down"></span><br/> </a> 
      ...
      <ul id="systemSetting" class="nav nav-pills nav-stacked collapse in">
        <li class="active"><a href="manage/usermanage"><i class="glyphicon glyphicon-user"></i>用户管理</a></li>
...
<c:forEach var="user" items="${requestScope.alluser}">
		<tr>
          ...
          <td>${user.email}</td>
          <td class="td-manage">
          <a title="删除" href="manage/userdelete?id=${user.id}"  class="btn btn-xs btn-warning" ><i class="icon-trash  bigger-120"></i>删除</a>
          </td>
		</tr>
</c:forEach>		
