 ...
 <!--data-toggle collapse 折叠  ul class =collapse 初始折叠 -->
    ...
    <i class="glyphicon glyphicon-cog"></i>后台管理 <span class="pull-right glyphicon glyphicon-chevron-down"></span><br/> </a> 
      ...
      <ul id="systemSetting" class="nav nav-pills nav-stacked collapse in">
        ...
        <li  class="active"> <a href="manage/messagemanage"> <i class="glyphicon glyphicon-barcode"></i>留言管理</a> </li>
...
<c:forEach var="message" items="${requestScope.allmessage}">
		<tr>
          <td><label><input type="checkbox" class="ace"><span class="lbl"></span></label></td>
          <td>${message.id}</td>
          <td>${message.houseid.name}</td>
          <td>${message.userid.username}</td>
          <td>${message.details}</td>
          <td>${message.time}</td>
          <td class="td-manage">
          <a title="删除" href="manage/messagedelete?id=${message.id}"  class="btn btn-xs btn-warning" ><i class="icon-trash  bigger-120"></i>删除</a>
          </td>
		</tr>
</c:forEach>		
